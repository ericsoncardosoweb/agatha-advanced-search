...
class AgathaAdvancedSearch {
	 ...
	 */
	function AgathaAdvancedSearch( $searchTerms, $displayPostMeta = true ) {
	...
	private function populate( $wpQueryResults, $displayPostMeta ) {
		...
		$wpQueryResults = apply_filters( 'aas_alter_results', $wpQueryResults, -1, $this );
		...
		while ( $wp_query->have_posts() ) {
			...
			if ( $displayPostMeta ) {
				...
				$authorName = apply_filters( 'aas_author_name', $authorName );
			...
			if ( $hasThumbnailSet ) {
				...
				$firstImageMeta = get_post_meta( $post->ID, '_aas_first_image', true );
				if ( $firstImageMeta ) {
					$post->attachment_thumbnail = $firstImageMeta;
				}
				else {
					// If no post thumbnail, grab the first image from the post_date
					$post->attachment_thumbnail = AAS_Util::updateFirstImagePostmeta( $post->ID, $post );
				}

			}

			$post->attachment_thumbnail = apply_filters( 'aas_attachment_thumbnail', $post->attachment_thumbnail );

			$post->post_excerpt = $this->excerpt( $post );

			$post->post_date = date_i18n( $dateFormat, strtotime( $post->post_date ) );
			$post->post_date = apply_filters( 'aas_post_date', $post->post_date );
			...
			$post->post_title = apply_filters( 'aas_post_title', $post->post_title );

			$post->show_more = true;

			// Let plugins & themes define their own template variables
			$post = apply_filters( 'aas_post_custom', $post );
	...
	private function excerpt( $result ) {
		...
		$excerpt = apply_filters( 'aas_the_excerpt', $excerpt );
	...
	public static function ajaxSearch() {
		...
		// This needs to be registered here so it's only invoked when processing a AAS AJAX request
		add_action( 'pre_get_posts', array( "AgathaAdvancedSearch", "pre_get_posts" ) );

		$displayPostMeta = (bool) get_option( 'agatha-advanced-search_display_post_meta' );
		$results = new AgathaAdvancedSearch( $_GET['s'], $displayPostMeta );
	...
	public static function pre_get_posts( $query ) {
		...
		$query_fields = apply_filters( 'aas_query_fields', $query_fields );
...
add_action( "wp_ajax_aas_search", array( "AgathaAdvancedSearch", "ajaxSearch" ) );
add_action( "wp_ajax_nopriv_aas_search", array( "AgathaAdvancedSearch", "ajaxSearch" ) );
