 ...
 * @param                                 $deprecated
 * @param AgathaAdvancedSearch $agathaAdvancedSearch
 *
 * @return mixed
 */
function add_liveblog_flag( $wpQueryResults, $deprecated, $agathaAdvancedSearch ) {
...
add_filter( 'aas_alter_results', 'add_liveblog_flag', 10, 3 );

/**
 * aas_attachment_thumbnail example.
 *
...
add_filter( 'aas_attachment_thumbnail', 'every_thumbnail_mickey' );

/**
 * aas_post_title example.
 *
...
add_filter( 'aas_post_title', 'all_titles_uppercase' );

/**
 * aas_the_excerpt example.
 *
...
add_filter( 'aas_the_excerpt', 'really_short_excerpt' );

/**
 * aas_post_date example.
 *
...
add_filter( 'aas_post_date', 'over_the_moon' );

/**
 * aas_author_name example.
 *
...
add_filter( 'aas_author_name', 'kilroy_was_here' );

/**
 * aas_alter_result_template example.
 *
 * The default markup used by Ericson Cardoso WordPress Live Search is ok, but I'd really prefer if it looked a little cooler.
...
function my_custom_results_template( $template ) {
	...
	/**
	 * The original template file is in 'js/aas-results.tpl' and is coded to work with the Underscore.js library.
...
add_filter( 'aas_alter_result_template', 'my_custom_results_template' );

/**
 * aas_post_custom example.
 *
 * Let's say the template from my 'aas_alter_result_template' example above includes a field for the weather. This
 ...
 * @return WP_Post
 */
function my_aas_template_variables( $post ) {
...
add_filter( 'aas_post_custom', 'my_aas_template_variables' );

/**
 * aas_query_fields example.
 *
 * In order to keep memory usage low and speed up the Live Search overall, Ericson Cardoso WordPress Live Search only retrieves
...
add_filter( 'aas_query_fields', 'i_want_more_fields' );